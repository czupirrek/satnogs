...
if [ "${CMD^^}" == "START" ]; then
  ...
  case "$NORAD" in
    ...
    "33591") # NOAA 19
      ...
      ;;
    "57166") # Meteor M2-3
      OPT="live meteor_m2-x_lrpt $OUT --source net_source --mode udp --port $UDP_DUMP_PORT --satellite_number 3 --samplerate $SAMP --frequency 0"
      ;;
    "59051") # Meteor M2-4
      OPT="live meteor_m2-x_lrpt $OUT --source net_source --mode udp --port $UDP_DUMP_PORT --satellite_number 4 --samplerate $SAMP --frequency 0"
      ;;
  esac
