...
# wywolanie satdumpa
satdump meteor_m2-x_lrpt baseband "$OUTPUT_WAV" "$OUT" --satellite-number "$SAT_NUMBER" &> "$OUT".log || true
...
# Sprawdzenie i usunięcie plików *map*
if ls "$OUT"/MSU-MR/*map* 1> /dev/null 2>&1; then
    rm "$OUT"/MSU-MR/*map*
fi

# Sprawdzenie i usunięcie plików *projected*
if ls "$OUT"/MSU-MR/*projected* 1> /dev/null 2>&1; then
    rm "$OUT"/MSU-MR/*projected*
fi

echo "[meteor pipeline] old files removed"

# etap 4 - wysłanie największego pliku do satnogs
FILE=$(find "$OUT"/MSU-MR -maxdepth 1 -type f -name '*.png' -printf "%s %p\n" | sort -nr | head -n1 | cut -d' ' -f2-)

if [ -n "$FILE" ] && [ -f "$FILE" ]; then
    mv "$FILE" "$SATNOGS_OUTPUT_PATH"/data_"$ID"_"$DATE".png
    echo "[meteor pipeline] plik przeniesiony pomyślnie"
else
    echo "[meteor pipeline] brak pliku PNG do przeniesienia" >&2
fi
